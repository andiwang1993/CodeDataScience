...
source("./of_wrapper.R")
source("./lbfgs.R")
#source("./lbfgs_restart.R")
source("./cmaes.R")
#source("./EGO.R")
# source("./krig_mean_aa_function.R")
# source("./KNF.R")
...
nameoffun <- "exe2" ## objective function, "quadratic", 
...
nameofoptim <- "random_search" # "random_search" , "normal_search" , "lbfgs", "EGO" , "cmaes"
budget <- 500 #1500 #70*zdim
no_test <<- 10
if (nameofoptim=="random_search") {
  ...
  param <- list(LB=-5,UB = 5,budget = budget,dim=zdim, xinit=rep(-4,zdim),sigma=1) # param for normal_search
  ...
  param <- list(LB=-5,UB = 5,budget = budget, dim=zdim, xinit=rep(-4, zdim),sigma=0.1) # param for cmaes  
} else if  (nameofoptim=="EGO") {
  param <- list(LB=-5,UB = 5,budget = budget,dim=zdim) # param for EGO
} else {
