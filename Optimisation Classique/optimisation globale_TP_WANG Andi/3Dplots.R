...
##### user data #####
nameoffun <- "sphere" # "quadratic", "ackley", "michalewicz", "sphere", "rastrigin", "schwefel", "tunnel" ...
nameofoptim <- "normal_search" # "random_search" , "normal_search", "lbfgs", "cmaes"
...
if (nameofoptim == "random_search") {
  ...
  param <- list(LB=-5,UB = 5,budget = budget,dim=zdim, xinit=c(-4,8),sigma=900) # param for normal_search
  ...
  param <- list(LB=-5,UB = 5,maxit = 100,dim=zdim, xinit=c(-2,6), trace=5, lmm =1, factr=100,pgtol=1.e-2) # param for lbfgs
} else {
